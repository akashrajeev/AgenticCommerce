...
export const purchaseIntentSchema = z.object({ merchantId: z.string().min(1), productId: z.string().min(1), quantity: z.number().int().positive().max(10), lineItems: z.array(purchaseIntentLineItemSchema).min(1).max(10).optional(), maxSpendPaise: z.number().int().nonnegative(), reason: z.string().min(1).max(500), quoteId: z.string().min(1) });
