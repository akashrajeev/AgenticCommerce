...
export type HealthStatus = { app: AppName; status: "ok"; timestamp: string };
export type MoneyAmount = { currency: "INR"; amountPaise: number };

export type Product = {
  id: string; sku: string; name: string; slug: string; category: string; pricePaise: number; currency: "INR";
  rating: number; reviewCount: number; inventory: number; shortDescription: string; description: string;
  features: string[]; specifications: Record<string, string>; tags: string[];
};
...
export type CheckoutQuote = {
  quoteId: string; merchantId: string; lineItems: CheckoutLineItem[]; subtotalPaise: number; shippingPaise: number;
  taxPaise: number; discountPaise: number; totalPaise: number; currency: "INR"; expiresAt: string;
};

export type PurchaseIntentLineItem = { productId: string; quantity: number };
export type PurchaseIntent = {
  id: string; merchantId: string; productId: string; quantity: number; lineItems?: PurchaseIntentLineItem[];
  maxSpendPaise: number; reason: string; quoteId: string;
};
export type PolicyCheck = { rule: string; result: "PASS" | "FAIL"; observed: string; expected: string; reason: string };
export type PolicyDecision = { decision: "ALLOW" | "BLOCK"; checks: PolicyCheck[]; evaluatedAt: string };
