...
export const agentCommerceCapabilities: AgentCommerceCapabilityDocument = {
  ...
  capabilities: [
    ...
    { capabilityId: "acp-checkout-sessions", name: "ACP-compatible checkout session lifecycle", version: "2026-04-17", protocol: "acp", methods: ["POST", "GET"], endpoint: "/checkout_sessions", requiresAuthentication: true },
  ...
  endpoints: [
    ...
    { capabilityId: "acp-checkout-create", method: "POST", path: "/checkout_sessions", description: "ACP-shaped authoritative checkout session creation backed by live merchant catalog and inventory.", requiresAuthentication: true },
    { capabilityId: "acp-checkout-update", method: "POST", path: "/checkout_sessions/:id", description: "ACP-shaped authoritative checkout session update with a fresh merchant quote.", requiresAuthentication: true },
    { capabilityId: "acp-checkout-retrieve", method: "GET", path: "/checkout_sessions/:id", description: "ACP-shaped authoritative checkout session retrieval.", requiresAuthentication: true },
    { capabilityId: "acp-checkout-complete", method: "POST", path: "/checkout_sessions/:id/complete", description: "ACP completion gate; MANDATE authorization is required before payment completion.", requiresAuthentication: true },
    { capabilityId: "acp-checkout-cancel", method: "POST", path: "/checkout_sessions/:id/cancel", description: "ACP-shaped checkout cancellation.", requiresAuthentication: true },
  ...
  supportedExtensions: ["multi-line-baskets", "merchant-recommendations", "deterministic-policy-gate", "audit-trail", "acp-checkout-session"],
  discovery: {
