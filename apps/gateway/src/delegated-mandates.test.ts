...
function product(id = "hp-001"): Product {
  return {
    id,
    sku: id,
    name: id === "hp-001" ? "SoundMax Pro" : "Arc Precision Mouse",
    slug: id,
    category: id === "hp-001" ? "headphones" : "mice",
    pricePaise: id === "hp-001" ? 399900 : 249900,
    currency: "INR",
    rating: 4.7,
    reviewCount: 20,
    inventory: 20,
...
test("allows multiple purchases while enforcing a reusable total budget", async () => {
  await withMerchantFetch(async () => {
    ...
    const second = await executeDelegatedMandate(mandate.mandateId, checkout());
    assert.equal(second.authorization.transaction.state, "policy_authorized");
  });
});

test("blocks merchant and product violations without creating an authorization", async () => {
  await withMerchantFetch(async () => {
    ...
    assert.equal(getTransaction(firstTransactionId(mandate.mandateId))?.mandateAuthorization, undefined);
    assert.equal(delegatedMandateStats(mandate.mandateId)?.blockedCount, 2);
  });
});

function firstTransactionId(_mandateId: string): string {
  return "txn_should_not_exist";
}
