...
function quote(): CheckoutQuote {
  const item = product();
  return {
    quoteId: "quote_negotiation_001",
    merchantId: "mandate-market",
    lineItems: [{ productId: item.id, quantity: 1, unitPricePaise: item.pricePaise, lineTotalPaise: item.pricePaise }],
    subtotalPaise: item.pricePaise,
    shippingPaise: 0,
    taxPaise: 71982,
    discountPaise: 0,
    totalPaise: 471882,
...
function offer(): MerchantOffer {
  const q = quote();
  return {
    offerId: "offer_negotiation_001",
    intentId: "intent_negotiation_001",
    merchantId: q.merchantId,
    items: q.lineItems,
    amount: { currency: "INR", amountPaise: q.totalPaise },
    quoteId: q.quoteId,
...
test("replays the same offer idempotently but rejects an amount tamper", async () => {
  await withMerchantFetch(async () => {
    const mandate = await createDelegatedMandate({
      subjectId: "user_001",
      agentId: "buyer-agent:mandate-demo",
    ...
    const tampered = { ...offer(), offerId: "offer_negotiation_002", amount: { currency: "INR" as const, amountPaise: 481882 } };
    await assert.rejects(() => acceptNegotiatedOffer({ negotiationId: "neg_002", mandateId: mandate.mandateId, offer: tampered }), /NEGOTIATED_OFFER_AMOUNT_MISMATCH/);
  });
});
