...
export function createApp() {
  ...
  app.get("/health", (_request, response) => response.json(createHealthStatus("gateway")));
  app.get("/", (_request, response) => response.json({ name: "MANDATE Gateway", role: "deterministic policy, transaction, Razorpay, webhook, and audit boundary", status: "razorpay-test-ready", testMode: true }));
  ...
  app.post("/v1/purchase-intents", async (request, response) => {
    try {
      response.status(201).json({ transaction: await proposeTransaction(parsePurchaseIntent(request.body)) });
    } catch (error) {
  ...
  app.post("/v1/transactions/:id/razorpay-order", async (request, response) => {
    try {
      ...
      if (transaction.state !== "policy_authorized") {
        if (transaction.razorpayOrderId) return response.json({ transaction, razorpayOrderId: transaction.razorpayOrderId, checkout: getPublicConfig() });
  ...
  app.post("/v1/transactions/:id/capture", async (request, response) => {
    try {
      ...
      return response.status(422).json({ error: error instanceof Error ? error.message : "CAPTURE_FAILED" });
    }
  });
