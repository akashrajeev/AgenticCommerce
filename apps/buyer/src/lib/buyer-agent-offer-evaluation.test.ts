...
function context(maxSpendPaise = 700000) {
  ...
  workspace.recommendations = [{
    ...
    type: "CROSS_SELL",
    score: 0.9,
    rationale: "Useful protection for travel",
    incrementalRevenuePaise: 49900,
  }];
...
test("accepts a merchant offer only when the offer is observed, in stock and within budget", () => {
  const ctx = context();
  const result = executeBuyerAgentOfferEvaluationTool(ctx, {
    productId: "case-001",
    decision: "accept",
    reason: "Useful protection and comfortably inside the user's limit.",
  });
...
test("supports explicit rejection without mutating payment state", () => {
  ...
  const result = executeBuyerAgentOfferEvaluationTool(ctx, {
    productId: "case-001",
    decision: "reject",
    reason: "The accessory is not necessary for this request.",
  });
...
test("rejects accepting an unobserved merchant offer", () => {
  const ctx = context();
  assert.throws(
    () => executeBuyerAgentOfferEvaluationTool(ctx, { productId: "unknown", decision: "accept", reason: "Useful" }),
    /BUYER_AGENT_OFFER_NOT_OBSERVED/,
  );
});

test("rejects accepting an offer that exceeds the immutable buyer limit", () => {
  const ctx = context(400000);
  assert.throws(
    () => executeBuyerAgentOfferEvaluationTool(ctx, { productId: "case-001", decision: "accept", reason: "Useful" }),
    /BUYER_AGENT_BUDGET_EXCEEDED/,
  );
});
