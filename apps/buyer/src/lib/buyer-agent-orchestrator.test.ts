...
import {
  ...
  runBuyerAgent,
  type BuyerAgentPlanner,
} from "./buyer-agent-orchestrator";

function mockFetch() {
  const original = globalThis.fetch;
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    const url = typeof input === "string" ? input : input instanceof URL ? input.toString() : input.url;
    if (url.endsWith("/.well-known/agent-commerce")) {
      return new Response(JSON.stringify({
        merchantId: "mandate-market",
        name: "Mandate Market",
        version: "1.3",
        currency: "INR",
        capabilities: { catalog: true, inventory: true, checkoutPreview: true },
        endpoints: { catalog: "/api/agent/catalog", product: "/api/agent/products/:id", inventory: "/api/agent/inventory/:id", checkoutPreview: "/api/agent/checkout/preview" },
      }), { status: 200, headers: { "content-type": "application/json" } });
    }
    if (url.endsWith("/api/agent/catalog")) {
    ...
    if (url.includes("/api/agent/inventory/")) {
      return new Response(JSON.stringify({ inventory: 4 }), { status: 200, headers: { "content-type": "application/json" } });
    }
    if (url.endsWith("/api/agent/checkout/preview")) {
      ...
      const totalPaise = (body.lineItems ?? []).reduce((sum, item) => sum + (item.productId === "hp-002" ? 579900 : 619900) * item.quantity, 0);
      return new Response(JSON.stringify({ quote: {
        quoteId: `quote_${Date.now()}`,
        merchantId: "mandate-market",
        lineItems: (body.lineItems ?? []).map((item) => ({ ...item, unitPricePaise: item.productId === "hp-002" ? 579900 : 619900, lineTotalPaise: (item.productId === "hp-002" ? 579900 : 619900) * item.quantity })),
...
test("model-directed buyer loop reaches approval with one tool per turn", async () => {
  ...
  try {
    ...
    const planner: BuyerAgentPlanner = async () => {
      ...
      return {
        tool,
        arguments: tool === "search_products"
...
test("a tool failure is returned to the planner and the agent can recover", async () => {
  ...
  try {
    ...
    const execution = await runBuyerAgent({ objective: "Recover from unavailable catalog", maxSpendPaise: 700000, planner, merchantInternalUrl: "http://merchant.test" });
    assert.equal(execution.run.state, "WAITING_FOR_APPROVAL");
...
test("planner cannot smuggle a payment tool into the execution loop", async () => {
  const planner = (async () => ({
    ...
    model: "test-planner",
    outputItems: [],
  })) as unknown as BuyerAgentPlanner;

  const execution = await runBuyerAgent({ objective: "Never call payment", maxSpendPaise: 700000, planner });
