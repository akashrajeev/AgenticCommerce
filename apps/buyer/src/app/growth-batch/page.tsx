...
type BatchResult = { transactionId: string; ok: boolean; body?: unknown; error?: string; order?: RazorpayOrder; checkout?: RazorpayCheckout };
...
export default function GrowthBatchPage() {
  ...
  function openRazorpay(result: BatchResult) {
    ...
    const instance = new window.Razorpay({
      ...
      name: "Mandate Market",
      description: "Merchant growth test cohort",
  ...
  return <>
    ...
    <main style={{ minHeight: "100vh", background: "#f5f5f2", color: "#171717", fontFamily: "Arial, sans-serif", padding: 36 }}><div style={{ maxWidth: 1080, margin: "0 auto" }}>
      ...
      {results.length > 0 ? <section style={{ marginTop: 14, background: "white", border: "1px solid #dddcd7", padding: 20 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "end", gap: 16 }}><div><div style={{ fontSize: 10, color: "#777", letterSpacing: ".1em", fontWeight: 800 }}>TEST MODE EXECUTION</div><h2 style={{ margin: "7px 0 0", fontSize: 25 }}>{successfulOrders.length} real Razorpay order{successfulOrders.length === 1 ? "" : "s"} ready for checkout</h2></div><div style={{ textAlign: "right", color: "#666", fontSize: 11 }}>{confirmedCount} confirmed · {pendingPaymentCount} awaiting payment/reconciliation</div></div><div style={{ marginTop: 14, display: "grid", gap: 10 }}>{results.map((result) => <div key={result.transactionId} style={{ border: "1px solid #e4e4e0", padding: 14, display: "grid", gridTemplateColumns: "1fr auto auto", gap: 12, alignItems: "center", fontSize: 11 }}><div><div style={{ fontFamily: "monospace", wordBreak: "break-all" }}>{result.transactionId}</div><div style={{ color: "#777", marginTop: 4 }}>{statuses[result.transactionId] ?? (result.ok ? "ORDER_CREATED" : result.error ?? "FAILED")}</div></div>{result.ok && result.order ? <strong>{money(result.order.amount)}</strong> : <span />}{result.ok && result.checkout ? <button disabled={activeTransaction !== "" || statuses[result.transactionId] === "ORDER_CONFIRMED"} onClick={() => openRazorpay(result)} style={{ border: 0, background: "#1e5b3e", color: "white", padding: "9px 12px", fontWeight: 800, cursor: "pointer" }}>{statuses[result.transactionId] === "ORDER_CONFIRMED" ? "Confirmed" : activeTransaction === result.transactionId ? "Opening…" : "Pay in Test Mode →"}</button> : <strong style={{ color: "#8d332d" }}>{result.error ?? "FAILED"}</strong>}</div>)}</div><div style={{ marginTop: 13, color: "#777", fontSize: 11 }}>Complete the Test Mode payments here. `/growth` independently re-reads captured Razorpay order/payment state before counting realized uplift.</div></section> : null}

      <section style={{ marginTop: 14, background: "#171717", color: "white", padding: 20 }}><div style={{ fontSize: 10, letterSpacing: ".1em", color: "#aaa", fontWeight: 800 }}>COHORT EVIDENCE</div><div style={{ marginTop: 9, color: "#c8c8c4", lineHeight: 1.6, fontSize: 12 }}>MANDATE authorization → MCP order creation → Razorpay Test Mode Checkout → gateway signature verification → webhook/API reconciliation → merchant order → independent Razorpay settlement proof.</div></section>
    </div></main>
  </>;
}
