...
export async function POST(request: Request) {
  ...
  if (action === "facilitator") {
    try {
      ...
      return NextResponse.json({ status: 502, body: { error: error instanceof Error ? error.message : "X402_FACILITATOR_PROBE_FAILED" } });
    }
  }
  ...
  if (action === "proof") {
    try {
      ...
      return NextResponse.json({ status: 502, body: { error: error instanceof Error ? error.message : "X402_PROOF_LOOKUP_FAILED" } });
    }
  }

  if (action === "live-settle") {
    if (!body?.paymentPayload || typeof body.paymentPayload !== "object" || Array.isArray(body.paymentPayload)) {
      return NextResponse.json({ status: 400, body: { error: "PAYMENT_PAYLOAD_REQUIRED" } }, { status: 400 });
    }
    try {
      const mandateResponse = await fetch(`${X402_SERVICE_URL}/demo/mandate`, { cache: "no-store" });
      const mandateBody = await mandateResponse.json().catch(() => null) as { mandate?: unknown; error?: unknown } | null;
      if (!mandateResponse.ok || !mandateBody?.mandate) {
        return NextResponse.json({ status: mandateResponse.status, body: mandateBody ?? { error: "X402_DEMO_MANDATE_UNAVAILABLE" } });
      }
      ...
      const response = await fetch(`${X402_SERVICE_URL}/resource`, {
        method: "GET",
        headers: {
          "payment-signature": paymentHeader,
          "x-mandate-service-mandate": mandateHeader,
        },
        cache: "no-store",
      });
      ...
      return NextResponse.json({
        ...
        demoCredential: { enabled: true, issuer: "ephemeral self-signed local demo credential" },
      });
    } catch (error) {
      return NextResponse.json({ status: 502, body: { error: error instanceof Error ? error.message : "X402_LIVE_SETTLEMENT_FAILED" } });
    }
  }
