...
export default function NegotiationPage() {
  ...
  async function createRazorpayOrder() {
    ...
    setBusy(true); setMessage("");
    try {
      const response = await fetch(`${gateway}/v1/transactions/${transaction.id}/razorpay-order`, { method: "POST" });
      const body = await response.json();
      ...
      setTransaction(body.transaction);
      setMessage(`Razorpay Test Mode order ${body.order.id} is ready. The agent still cannot finalize payment by itself.`);
  ...
  function openRazorpay() {
    ...
    const instance = new RazorpayCtor({
      ...
      handler: async (response: { razorpay_payment_id: string; razorpay_order_id: string; razorpay_signature: string }) => {
        ...
        setTransaction(body.transaction);
        setMessage(body.verified ? "Payment verified by the gateway." : "Payment returned and is awaiting final reconciliation.");
      },
    });
    instance.open();
  }
    ...
    <main className={styles.shell}>
      ...
      <section className={styles.grid}>
        <section className={styles.left}>
          <span className={styles.kicker}>PHASE 6 / AGENT-TO-AGENT COMMERCE</span>
          <h1>Let two agents negotiate. Keep payment deterministic.</h1>
          <p className={styles.lede}>The buyer agent states the intent. The merchant agent returns quote-backed offers. The trusted gateway decides whether the selected offer fits the user&apos;s delegated authority.</p>
          <div className={styles.agentStrip}><span>BUYER AGENT</span><b>↔</b><span>MERCHANT AGENT</span><b>→</b><span>MANDATE</span></div>
        ...
        <section className={styles.right}>
          <div className={styles.rightHead}><div><span className={styles.panelKicker}>NEGOTIATION TRACE</span><h2>{negotiation ? negotiation.negotiationId : "No negotiation yet"}</h2></div>{negotiation && <span className={styles.status}>PROPOSED</span>}</div>
          {negotiation ? <>
            ...
            {transaction && <div className={styles.authorized}><div><span>MANDATE GATEWAY</span><strong>Offer accepted · {money(transaction.quote.totalPaise)}</strong><small>{transaction.id}</small></div>{!checkout ? <button onClick={() => void createRazorpayOrder()} disabled={busy}>Create Test Order →</button> : <button onClick={openRazorpay} disabled={busy}>Open Razorpay Checkout ↗</button>}</div>}
          </> : <div className={styles.empty}><span>↔</span><p>Start a negotiation to see the buyer and merchant agents exchange an intent and live merchant offers.</p></div>}
        </section>
      </section>
    </main>
  );
