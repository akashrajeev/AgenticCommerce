...
export default function GoldenPathPage() {
  ...
  const [stage, setStage] = useState<"idle" | "planning" | "approval" | "authorizing" | "authorized" | "payment" | "paid" | "blocked">("idle");
  const [error, setError] = useState("");

  const basket = useMemo(() => product ? [{ productId: product.id, quantity: 1 }, ...recommendations.map((item) => ({ productId: item.productId, quantity: item.quantity }))] : [], [product, recommendations]);
  const upliftPaise = transaction ? Math.max(transaction.quote.totalPaise - (product?.pricePaise ?? 0), 0) : recommendations.reduce((sum, item) => sum + (item.pricePaise ?? 0) * item.quantity, 0);
  ...
  async function buildPlan() {
    ...
    try {
      ...
      const recommendationResponse = await fetch(`/api/golden-path/recommendations?productId=${encodeURIComponent(body.selectedProduct.id)}&maxSpendPaise=${encodeURIComponent(String(maxSpend * 100))}`, { cache: "no-store" });
      const recommendationBody = await recommendationResponse.json() as { recommendations?: Recommendation[]; error?: string };
      if (recommendationResponse.ok) {
        setRecommendations((recommendationBody.recommendations ?? []).filter((item) => item.productId !== body.selectedProduct!.id).slice(0, 2));
      }
      setStage("approval");
    } catch (caught) {
  ...
  async function authorize(includeAddOn: boolean) {
    if (!product) return;
    const selectedRecommendations = includeAddOn ? recommendations.slice(0, 1) : [];
    const lines = [{ productId: product.id, quantity: 1 }, ...selectedRecommendations.map((item) => ({ productId: item.productId, quantity: item.quantity }))];
    ...
    try {
      ...
      const body = await response.json() as { transaction?: Transaction; quote?: Quote; error?: string };
  ...
  function launchCheckout() {
    ...
    const instance = new RazorpayCtor({ key: keyId, amount: order.amount, currency: order.currency, name: "Mandate Market", description: request, order_id: order.id, theme: { color: "#171717" }, handler: async (response: { razorpay_payment_id: string; razorpay_order_id: string; razorpay_signature: string }) => {
      ...
      if (!verifyResponse.ok || !body.transaction) { setError(body.error ?? "Payment verification failed."); return; }
  ...
  return <main style={{ minHeight: "100vh", background: "#f7f7f3", color: "#171717", fontFamily: "Arial, sans-serif", padding: "34px 20px 70px" }}>
    ...
    <div style={{ maxWidth: 1120, margin: "0 auto" }}>
      ...
      <section style={{ marginTop: 18, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>{[["01", "AI PLAN", ["planning", "approval", "authorizing", "authorized", "payment", "paid", "blocked"].includes(stage) ], ["02", "USER APPROVAL", approved || stage === "authorized" || stage === "payment" || stage === "paid" || stage === "blocked"], ["03", "POLICY GATE", stage === "authorized" || stage === "payment" || stage === "paid" || stage === "blocked"], ["04", "PAYMENT", stage === "paid" || stage === "payment"]].map(([number, label, complete]) => <div key={number} style={{ background: complete ? "#eff6f1" : "white", border: `1px solid ${complete ? "#c5d6ca" : "#dddcd7"}`, padding: 16 }}><div style={{ fontSize: 10, color: "#777", letterSpacing: ".08em", fontWeight: 700 }}>{number}</div><div style={{ fontSize: 12, fontWeight: 700, marginTop: 7 }}>{label}</div><div style={{ marginTop: 12, fontSize: 20, color: complete ? "#1e5b3e" : "#aaa" }}>{complete ? "✓" : "○"}</div></div>)}</section>

      {plan && product ? <section style={{ marginTop: 18, background: "white", border: "1px solid #dddcd7", padding: 22 }}><div style={{ fontSize: 10, color: "#777", letterSpacing: ".1em", fontWeight: 700 }}>AI BUYER + MERCHANT REVENUE OPPORTUNITY</div><div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "1fr .95fr", gap: 22 }}><div><div style={{ fontSize: 28, fontWeight: 700 }}>{product.name}</div><div style={{ marginTop: 5, color: "#666", fontSize: 13 }}>{plan.decisionSummary}</div><div style={{ display: "flex", gap: 18, marginTop: 16, fontSize: 12 }}><span>Rating <strong>{product.rating}</strong></span><span>Inventory <strong>{product.inventory}</strong></span><span>Price <strong>{money(product.pricePaise)}</strong></span></div></div><div style={{ borderLeft: "1px solid #e8e7e2", paddingLeft: 20 }}><div style={{ fontSize: 10, color: "#777", letterSpacing: ".08em", fontWeight: 700 }}>MERCHANT OPPORTUNITY</div>{recommendations.length ? <><div style={{ fontSize: 16, fontWeight: 700, marginTop: 8 }}>Add an accessory?</div><div style={{ marginTop: 7, fontSize: 12, color: "#666" }}>{recommendations[0].productId} · quantity {recommendations[0].quantity}</div><div style={{ marginTop: 7, fontSize: 12, color: "#555" }}>{recommendations[0].rationale}</div></> : <div style={{ marginTop: 8, fontSize: 12, color: "#666" }}>No additional merchant opportunity returned for this product.</div>}</div></div></section> : null}

      {product && plan && stage === "approval" ? <section style={{ marginTop: 18, background: "#fffdf3", border: "1px solid #e4d9af", padding: 20 }}><div style={{ fontSize: 10, color: "#75672c", letterSpacing: ".1em", fontWeight: 700 }}>EXPLICIT USER APPROVAL</div><div style={{ fontSize: 20, fontWeight: 700, marginTop: 8 }}>Choose what the buyer is allowed to purchase.</div><div style={{ color: "#6f6a57", fontSize: 12, marginTop: 7 }}>The recommendation is not automatically added to the purchase intent.</div><div style={{ display: "flex", gap: 10, marginTop: 16 }}><button onClick={() => void authorize(false)} style={{ border: "1px solid #aaa", background: "white", padding: "12px 15px", fontWeight: 700 }}>Primary product only →</button>{recommendations.length ? <button onClick={() => void authorize(true)} style={{ border: 0, background: "#171717", color: "white", padding: "12px 15px", fontWeight: 700 }}>Approve add-on →</button> : null}</div></section> : null}

      {transaction ? <section style={{ marginTop: 18, background: "white", border: "1px solid #dddcd7" }}><div style={{ padding: 20, borderBottom: "1px solid #e8e7e2", display: "flex", justifyContent: "space-between", gap: 18 }}><div><div style={{ fontSize: 10, color: "#777", letterSpacing: ".1em", fontWeight: 700 }}>GATEWAY DECISION</div><div style={{ fontSize: 27, fontWeight: 700, marginTop: 8 }}>{stage === "blocked" ? "Basket blocked before Razorpay" : stage === "paid" ? "Payment verified" : "Policy authorized"}</div><div style={{ marginTop: 5, color: "#777", fontFamily: "monospace", fontSize: 11 }}>{transaction.id}</div></div><span style={{ padding: "8px 10px", background: stage === "blocked" ? "#f7e7e4" : "#eaf4ed", color: stage === "blocked" ? "#8d332d" : "#1e5b3e", height: "fit-content", fontSize: 11, fontWeight: 700 }}>{transaction.state.replaceAll("_", " ")}</span></div><div style={{ padding: 20, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}><div><div style={{ fontSize: 10, color: "#777", letterSpacing: ".08em", fontWeight: 700 }}>BASKET ECONOMICS</div><div style={{ display: "flex", justifyContent: "space-between", marginTop: 12 }}><span>Final quote</span><strong>{money(transaction.quote.totalPaise)}</strong></div><div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}><span>Hard limit</span><strong>{money(maxSpend * 100)}</strong></div><div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}><span>Incremental basket value</span><strong>{money(upliftPaise)}</strong></div></div><div><div style={{ fontSize: 10, color: "#777", letterSpacing: ".08em", fontWeight: 700 }}>POLICY CHECKS</div>{transaction.policy?.checks.map((check) => <div key={check.rule} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #eeeeea", fontSize: 11 }}><span><strong>{check.result === "PASS" ? "✓" : "×"}</strong> {check.rule.replaceAll("_", " ")}</span><code>{check.observed}</code></div>)}</div></div><div style={{ padding: 20, background: "#fafaf7", borderTop: "1px solid #e8e7e2", display: "flex", justifyContent: "space-between", alignItems: "center" }}><div><strong>{stage === "blocked" ? "NO RAZORPAY ORDER" : order ? "RAZORPAY TEST ORDER READY" : "POLICY AUTHORIZED"}</strong><div style={{ color: "#777", fontSize: 11, marginTop: 5 }}>{order ? `${order.id} · ${money(order.amount)}` : stage === "blocked" ? "The payment rail was never reached." : "Create the Test Mode order only after authorization."}</div></div>{stage === "authorized" ? <button onClick={() => void createOrder()} style={{ border: 0, background: "#171717", color: "white", padding: "11px 14px", fontWeight: 700 }}>Create Razorpay Test Order →</button> : order && stage === "payment" ? <button onClick={launchCheckout} style={{ border: 0, background: "#171717", color: "white", padding: "11px 14px", fontWeight: 700 }}>Open Razorpay Checkout ↗</button> : null}</div></section> : null}

      <div style={{ marginTop: 24, paddingTop: 17, borderTop: "1px solid #d9d8d2", color: "#777", fontSize: 11, lineHeight: 1.7 }}>Canonical evidence chain: AI plan → merchant recommendation → user approval → live quote → deterministic policy → Razorpay Test Mode → server verification → webhook reconciliation → merchant order.</div>
    </div>
  </main>;
}
