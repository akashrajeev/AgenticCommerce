...
export default function BuyerWorkspace() {
  ...
  async function runAgent() {
    ...
    try {
      ...
      const planResponse = await fetch("/api/agent/purchase-plan", {
        method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ request, maxSpendPaise: maxSpend * 100 }),
      });
      ...
      const quoteResponse = await fetch(`${MERCHANT_URL}/api/agent/checkout/preview`, {
        method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ productId: chosen.id, quantity: 1 }),
      });
      ...
      const transactionResponse = await fetch(`${GATEWAY_URL}/v1/purchase-intents`, {
        ...
        body: JSON.stringify({ merchantId: "mandate-market", productId: chosen.id, quantity: 1, maxSpendPaise: maxSpend * 100, reason: request, quoteId: quoteBody.quote.quoteId }),
      });
      ...
      setStage("ready"); setError(caught instanceof Error ? caught.message : "The agent could not complete the request.");
    }
  }
  ...
  function launchRazorpay() {
    ...
    const instance = new RazorpayCtor({
      key: checkout.keyId, amount: checkout.order.amount, currency: checkout.currency, name: "Mandate Market",
      description: transaction.intent.reason, order_id: checkout.order.id, theme: { color: "#171717" },
      handler: async (response: { razorpay_payment_id: string; razorpay_order_id: string; razorpay_signature: string }) => {
        ...
        setTransaction(verifyBody.transaction); setStage(verifyBody.verified ? "paid" : "payment");
