...
type JudgePayload = { generatedAt: string; testMode: boolean; checks: Check[]; metrics: { authorizedTransactions: number; confirmedTransactions: number; merchantOrders: number; razorpayVerifiedPayments: number; protocolCount: number; growthUpliftPaise: number; campaignVerifiedPayments?: number; campaignVerifiedUpliftPaise?: number }; links: Record<string, string> };
...
export default function JudgeModePage() {
  ...
  return <main style={{ minHeight: "100vh", background: "#f4f4f1", color: "#171717", padding: "36px 20px 72px", fontFamily: "Inter, Arial, sans-serif" }}><div style={{ maxWidth: 1180, margin: "0 auto" }}>
    ...
    <section style={{ display: "grid", gridTemplateColumns: "1.1fr .9fr", gap: 16 }}>
      ...
      <div style={{ background: "white", border: "1px solid #dddcd7", padding: 24 }}><div style={{ fontSize: 10, color: "#777", letterSpacing: 1.5, fontWeight: 800 }}>REAL EVIDENCE COUNTS</div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 15 }}>{[["Authorized TX", data?.metrics.authorizedTransactions ?? 0], ["Confirmed TX", data?.metrics.confirmedTransactions ?? 0], ["Merchant orders", data?.metrics.merchantOrders ?? 0], ["Razorpay verified", data?.metrics.razorpayVerifiedPayments ?? 0], ["Protocols", data?.metrics.protocolCount ?? 0], ["Verified uplift", money(data?.metrics.growthUpliftPaise ?? 0)], ["Campaign payments", data?.metrics.campaignVerifiedPayments ?? 0], ["Campaign uplift", money(data?.metrics.campaignVerifiedUpliftPaise ?? 0)]].map(([label, value]) => <div key={label} style={{ border: "1px solid #e3e3df", padding: 15 }}><div style={{ fontSize: 9, color: "#777", letterSpacing: 1, fontWeight: 800 }}>{label}</div><div style={{ fontSize: 25, fontWeight: 900, marginTop: 7 }}>{value}</div></div>)}</div><div style={{ marginTop: 15, background: "#f7f7f4", border: "1px solid #e2e2de", padding: 14, fontFamily: "monospace", fontSize: 10, lineHeight: 1.7 }}>No captured Razorpay verification<br />→ no realized uplift<br /><br />No MANDATE authorization<br />→ no payment-changing tool execution</div></div>
    </section>

    <section style={{ marginTop: 16, background: "white", border: "1px solid #dddcd7", padding: 22 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 18, alignItems: "end" }}><div><div style={{ fontSize: 10, color: "#777", letterSpacing: 1.5, fontWeight: 800 }}>JUDGE ROUTES</div><h2 style={{ margin: "7px 0 0", fontSize: 25 }}>Run the proof, not the pitch.</h2></div><span style={{ color: "#777", fontSize: 10 }}>{data ? `Last sweep ${new Date(data.generatedAt).toLocaleTimeString()}` : "waiting"}</span></div><div style={{ display: "grid", gridTemplateColumns: "repeat(8, 1fr)", gap: 10, marginTop: 16 }}>{[["Flagship payment", String(data?.links?.negotiation ?? ""), "Agent negotiation → Test Mode Checkout"],["Growth", String(data?.links?.growth ?? ""), "Verified revenue uplift"],["Test cohort", String(data?.links?.growthBatch ?? ""), "Real Test Mode batch orders"],["Campaign", String(data?.links?.campaigns ?? ""), "Merchant agent growth"],["Attribution", String(data?.links?.campaignAttribution ?? ""), "Campaign-owned Test Mode uplift"],["MCP guard", String(data?.links?.mcp ?? ""), "Unauthorized → blocked / Authorized → Test Mode"],["Protocols", String(data?.links?.protocols ?? ""), "Live protocol matrix"],["Unified proof", String(data?.links?.proof ?? ""), "Live Razorpay evidence + audit chain"]].map(([label, href, detail]) => href ? <a key={label} href={href} style={{ textDecoration: "none", color: "#171717", border: "1px solid #e2e2de", padding: 16, background: "#fbfbf8" }}><strong style={{ display: "block", fontSize: 13 }}>{label}</strong><span style={{ display: "block", marginTop: 7, color: "#666", fontSize: 11, lineHeight: 1.5 }}>{detail}</span><span style={{ display: "block", marginTop: 12, fontSize: 11, fontWeight: 800 }}>Open →</span></a> : null)}</div></section>
