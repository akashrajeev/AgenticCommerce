...
export default function JudgeModePage() {
  ...
  return <main style={{ minHeight: "100vh", background: "#f4f4f1", color: "#171717", padding: "36px 20px 72px", fontFamily: "Inter, Arial, sans-serif" }}><div style={{ maxWidth: 1180, margin: "0 auto" }}>
    ...
    <section style={{ marginTop: 16, background: "white", border: "1px solid #dddcd7", padding: 22 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 18, alignItems: "end" }}><div><div style={{ fontSize: 10, color: "#777", letterSpacing: 1.5, fontWeight: 800 }}>JUDGE ROUTES</div><h2 style={{ margin: "7px 0 0", fontSize: 25 }}>Run the proof, not the pitch.</h2></div><span style={{ color: "#777", fontSize: 10 }}>{data ? `Last sweep ${new Date(data.generatedAt).toLocaleTimeString()}` : "waiting"}</span></div><div style={{ display: "grid", gridTemplateColumns: "repeat(8, 1fr)", gap: 10, marginTop: 16 }}>{[["Flagship payment", data?.links.negotiation, "Agent negotiation → Test Mode Checkout"],["Growth", data?.links.growth, "Verified revenue uplift"],["Test cohort", data?.links.growthBatch, "Real Test Mode batch orders"],["Campaign", data?.links.campaigns, "Merchant agent growth"],["Attribution", data?.links.campaignAttribution, "Campaign-owned Test Mode uplift"],["MCP guard", data?.links.mcp, "Unauthorized → blocked / Authorized → Test Mode"],["Protocols", data?.links.protocols, "Live protocol matrix"],["Unified proof", data?.links.proof, "Live Razorpay evidence + audit chain"]].map(([label, href, detail]) => href ? <a key={label} href={href} style={{ textDecoration: "none", color: "#171717", border: "1px solid #e2e2de", padding: 16, background: "#fbfbf8" }}><strong style={{ display: "block", fontSize: 13 }}>{label}</strong><span style={{ display: "block", marginTop: 7, color: "#666", fontSize: 11, lineHeight: 1.5 }}>{detail}</span><span style={{ display: "block", marginTop: 12, fontSize: 11, fontWeight: 800 }}>Open →</span></a> : null)}</div></section>
