    ...
    <main style={{ minHeight: "100vh", background: "#f4f1eb", color: "#171717", fontFamily: "Arial, Helvetica, sans-serif", padding: "34px 20px 70px" }}>
      <div style={{ maxWidth: 1180, margin: "0 auto" }}>
        ...
        <section style={{ marginTop: 22, display: "grid", gridTemplateColumns: "1.15fr .85fr", gap: 16 }}>
          ...
          <section style={{ background: "#171717", color: "white", borderRadius: 18, padding: 22 }}><div style={{ fontSize: 10, letterSpacing: ".12em", fontWeight: 800, color: "#aaa" }}>THE JUDGE FLOW</div><div style={{ display: "grid", gap: 10, marginTop: 15 }}>{[["01", "Discover", "/capabilities", "Merchant capability handshake"], ["02", "Negotiate", "/negotiation", "Buyer ↔ merchant agent"], ["03", "Authorize", "/delegated-mandates", "Bound autonomous spending"], ["04", "Pay", "/mcp/authorized", "MCP → MANDATE → Razorpay Test Mode"], ["05", "Measure", "/growth", "Verified merchant uplift"]].map(([n, title, href, detail]) => <Link key={n} href={href} style={{ display: "grid", gridTemplateColumns: "30px 1fr auto", gap: 11, alignItems: "center", color: "white", textDecoration: "none", border: "1px solid #393939", padding: 13, borderRadius: 12 }}><span style={{ color: "#999", fontFamily: "monospace", fontSize: 10 }}>{n}</span><div><strong style={{ display: "block", fontSize: 12 }}>{title}</strong><span style={{ display: "block", color: "#aaa", marginTop: 3, fontSize: 10 }}>{detail}</span></div><span style={{ color: "#c7c1b7" }}>→</span></Link>)}</div></section>
        </section>

        <section style={{ marginTop: 16, background: "#fffdf8", border: "1px solid #ddd6ca", borderRadius: 18, padding: 22 }}><div style={{ fontSize: 10, letterSpacing: ".12em", fontWeight: 800, color: "#777" }}>PRIORITY SURFACES</div><div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginTop: 14 }}>{[["Growth", "/growth", "Realized Test Mode uplift"], ["MCP proof", "/mcp/authorized", "Unauthorized vs authorized"], ["x402", "/x402", "Facilitator /verify → /settle"], ["Protocols", "/protocols", "Live capability/protocol probes"]].map(([title, href, detail]) => <Link key={title} href={href} style={{ textDecoration: "none", color: "inherit", border: "1px solid #e2dcd2", padding: 16, borderRadius: 14 }}><strong style={{ fontSize: 15 }}>{title}</strong><span style={{ display: "block", color: "#6f665d", fontSize: 11, lineHeight: 1.45, marginTop: 6 }}>{detail}</span><span style={{ display: "block", marginTop: 13, fontSize: 11, fontWeight: 800 }}>Open →</span></Link>)}</div></section>
